...
*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:var(--paper);color:var(--ink);font:16px/1.5 "Source Sans 3","Segoe UI",sans-serif}}a{{color:var(--primary);text-underline-offset:3px}}a:focus-visible,summary:focus-visible{{outline:0;box-shadow:var(--focus)}}code,time,td{{font-family:"IBM Plex Mono",Consolas,monospace;font-variant-numeric:tabular-nums}}.skip{{position:absolute;left:8px;top:8px;transform:translateY(-160%);background:var(--ink);color:var(--paper);padding:10px 14px}}.skip:focus{{transform:none}}header,main,footer{{width:min(calc(100% - 40px),1280px);margin:auto}}header{{padding:64px 0 32px;border-bottom:2px solid var(--ink)}}.kicker{{color:var(--primary);font:600 12px/1.4 "IBM Plex Mono",Consolas,monospace;letter-spacing:.08em;text-transform:uppercase}}h1,h2{{font-family:"Instrument Serif",Georgia,serif;font-weight:400}}h1{{font-size:clamp(42px,6vw,72px);line-height:1;margin:12px 0}}.meta{{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--border);border:1px solid var(--border);margin-top:28px}}.meta div{{background:var(--surface);padding:14px}}.meta span{{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}}.verdict{{display:flex;gap:14px;align-items:center;margin:32px 0;padding:20px;border-left:4px solid currentColor}}.verdict.passed{{color:var(--success);background:var(--success-soft)}}.verdict.failed{{color:var(--error);background:var(--error-soft)}}.verdict b{{font-size:20px}}.release-warning{{margin:-20px 0 32px;padding:14px;border-left:4px solid var(--warning);background:var(--warning-soft);color:var(--warning);font-weight:700}}section{{padding:40px 0;border-top:1px solid var(--border)}}h2{{font-size:36px;line-height:1;margin:0 0 20px}}.grid{{display:grid;grid-template-columns:5fr 7fr;gap:32px}}table{{width:100%;border-collapse:collapse}}caption{{text-align:left;color:var(--muted);padding-bottom:10px}}th,td{{padding:10px 8px;border-bottom:1px solid var(--border);text-align:left;vertical-align:top}}thead th{{font-size:12px;color:var(--muted);text-transform:uppercase}}tbody th{{font-weight:500}}.pass{{color:var(--success);font-weight:700}}.fail{{color:var(--error);font-weight:700}}.table-wrap{{overflow-x:auto}}ol{{list-style:none;margin:0;padding:0}}li{{display:grid;grid-template-columns:180px 1fr;gap:16px;padding:10px 0;border-bottom:1px solid var(--border)}}li span{{display:block;color:var(--muted)}}details{{margin-top:20px;border:1px solid var(--border);padding:12px}}summary{{min-height:44px;display:flex;align-items:center;cursor:pointer;font-weight:700}}footer{{padding:32px 0 64px;color:var(--muted);border-top:1px solid var(--border)}}
...
<div class="grid"><section aria-labelledby="checks"><h2 id="checks">Required invariants</h2><div class="table-wrap"><table><caption>Expected and actual values remain adjacent.</caption><thead><tr><th>Invariant</th><th>Expected</th><th>Actual</th><th>Result</th></tr></thead><tbody>{check_rows}</tbody></table></div></section>
