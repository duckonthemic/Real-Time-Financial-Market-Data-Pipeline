...
def test_fixture_matches_manifest(fixture: str) -> None:
    ...
    assert (
        sum(record.event is None for record in records) == manifest["rejection_counts"]["BAD_WIRE"]
    )
    assert canonical_event_set_sha256(manifest) == manifest["canonical_event_set_sha256"]
    event_ids = Counter(
        record.event["event_id"]
        for record in records
        if record.event and not record.event["event_id"].startswith("invalid")
    )
