...
from demo_support.artifacts import (
    DemoLock,
    artifact,
    atomic_write_json,
    read_artifact,
...
def test_run_schema_accepts_representative_artifact() -> None:
    schema = json.loads(
        (ROOT / "schemas" / "artifacts" / "run.schema.json").read_text(encoding="utf-8")
    )
    jsonschema.validate(
        artifact("run", "run-contract-01", state="CREATED", scenario="standard"), schema
    )
...
def test_lock_is_exclusive_and_reports_owner(tmp_path: Path) -> None:
    ...
    with (
        DemoLock(lock_path, "run-contract-01"),
        pytest.raises(ConfigFailure, match="run_id=run-contract-01"),
        DemoLock(lock_path, "run-contract-02"),
    ):
        pass
